...
//        mockMvc.perform(delete("/api/news/{id}", news.getId()).contentType("application/json;charset=UTF-8")).andExpect(status().isNotFound());
//
//    }
//
//
...
//        when(newsService.getNews("checkall")).thenReturn(news);
//        mockMvc.perform(get("/api/news/{userName}").contentType("application/json;charset=UTF-8")).andExpect(status().isOk());
//
//    }
}
